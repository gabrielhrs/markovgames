...
- **`Csg/`** — a from-scratch formalization of the multi-player
  ...
  values, backward induction for bounded objectives, quantitative
  reachability/until/reward-until/safety/Büchi/co-Büchi operators for the
  infinite-horizon case, qualitative (sure/almost-sure/limit-sure)
  winning-region algorithms for safety/reachability/co-Büchi/Büchi
  objectives, and fixed-point-certificate combinators (both exact-pinning
  and interval/sandwiched forms) for verifying computed values against
  those operators without re-deriving the operators' own correctness.
...
The qualitative line (`Csg/QualitativeSure.lean`,
`Csg/QualitativeAlmostSure.lean`, `Csg/QualitativeLimitSure.lean`) is done
too: sure/almost-sure/limit-sure winning regions for all four objective
shapes (safety `G`, reachability `F`, co-Büchi `FG`, Büchi `GF`), ported 
from PRISM-games' own `explicit/CSGModelChecker.java`. Validated against a
worked instance (`Csg/Skirmish.lean`/`Csg/SkirmishProperties.lean`, the
plain SKIRMISH game LICS 2000 itself uses to demonstrate the
sure/almost-sure-vs-limit-sure divergence): all 14 `sure`/`almost`/`limit`
facts, plus the three Büchi facts, from PRISM-games' own
`skirmish.prism.props` test fixture are reproduced exactly, covering both
headline divergences (reachability and Büchi) the original LICS 2000
paper is about.
...
- Luca de Alfaro and Thomas A. Henzinger,
  ["Concurrent Omega-Regular Games"](https://ieeexplore.ieee.org/abstract/document/855763/),
  LICS 2000. The source for the sure/almost-sure/limit-sure mode distinction and the qualitative
  winning-region algorithms in `Csg/QualitativeSure.lean`, `Csg/QualitativeAlmostSure.lean`, and
